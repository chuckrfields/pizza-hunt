...
function getPizza() {
  ...
  fetch(`/api/pizzas/${pizzaId}`)
    .then(response => {
      ...
      // check for error
      if (!response.ok) {
        throw new Error( { message: "Unable to get pizza by that id" });
      }
    ...
    .catch(err => {
      console.log(err);
      alert('Cannot find pizza by that id; taking you back.'); 
      window.history.back(); // As long as this particular browser session has a previous page, it will behave as if the user had clicked on the "Back" button.
    });
};
